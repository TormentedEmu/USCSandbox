...
namespace USCSandbox.Processor
{
    internal class ShaderProcessor
    {
        ...
        public string Process()
        {
            ...
            Directory.CreateDirectory(Path.Combine(Application.StartupPath, "BinaryFiles", name));
            var parsedFormFilePath = Path.Combine(Application.StartupPath, "BinaryFiles", $"{name}.txt");
            File.WriteAllBytes(parsedFormFilePath, parsedForm.WriteToByteArray());
            ...
            for (var i = 0; i < blobManager.Entries.Count; i++)
            {
                ...
                Directory.CreateDirectory(Path.Combine(Application.StartupPath, "BinaryFiles", name));
                var binaryFilePath = Path.Combine(Application.StartupPath, "BinaryFiles", name, $"dbg_blob_entry_{i}.bin");
                File.WriteAllBytes(binaryFilePath, entryBytes);
            }
        ...
        private void WritePassBody(BlobManager blobManager, List<ShaderProgramBasket> baskets, int depth)
        {
            ...
            foreach (var basket in basketsInfo)
            {
                ...
                Directory.CreateDirectory(Path.Combine(Application.StartupPath, "BinaryFiles", _shaderName));
                var binaryFilePath = Path.Combine(Application.StartupPath, "BinaryFiles", _shaderName, $"dbg_subprog_blob_entry_data_{subProgInfo.BlobIndex}.bin");
                File.WriteAllBytes(binaryFilePath, subProg.ProgramData);
                //File.WriteAllBytes($"dbg_subprog_blob_entry_data_{subProgInfo.BlobIndex}.bin", subProg.ProgramData);
